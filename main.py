...
# from src.FFOE.dataset_2D_APE import Dictionary, GQAFeatureDataset  # 2D-APE only
...
def parse_args():
    ...
    # 20250721: add flag to use the sweep in weight and biases to tune the hyperparameters
...
# def main():
#     wandb.init(project="CFR_VQA_PEmixer_2DRPE_alibi_localtest", name="sweepconfig1_PEmixer_2DRPE_20250806")

    args = parse_args()
    # args.lxmert_lr = args.lxmert_lr = getattr(wandb.config, 'lxmert_lr', args.lxmert_lr)
    # args.dropout = getattr(wandb.config, 'dropout', args.dropout)
    # args.omega_v = getattr(wandb.config, 'omega_v', args.omega_v)
    # args.omega_q = getattr(wandb.config, 'omega_q', args.omega_v)
    print("====================================================")
    print(f"args.lxmert_lr: {args.lxmert_lr}")
    print(f"args.dropout: {args.dropout}")
    print(f"args.omega_v: {args.omega_v}")
    print(f"args.omega_q: {args.omega_q}")

    utils.create_dir(args.output)
    logger = utils.Logger(os.path.join(args.output, 'log.txt'))
    logger.write(args.__repr__())  # record the parameters
    ...
    if args.use_both:  # use train & val splits to train the model, default is False
    ...
    # train(args, model, train_loader, eval_loader, args.epochs, args.output, optim, epoch, wandb.config)
...
#     #     # dictionary = Dictionary.load_from_file("/scratch/xcwx3620/MSc_Project/Codes/CFR_VQA-main-zhd/data/gqa/dictionary.pkl")
#     #     dictionary = Dictionary.load_from_file("D:/CFR_train_extract_0711/dictionary.pkl")
#     #     train_dset = GQAFeatureDataset(args, 'train', dictionary, adaptive=True, train_read=False)
#     #     # train_read = True
#     #     val_dset = GQAFeatureDataset(args, 'val', dictionary, adaptive=True, train_read=True, train_dset=train_dset)
#     #
#     #     # print(f"len(train_dset): {train_dset}")
#     #     # print(f"len(val_dset): {val_dset}")
#     #
#     # else:
#     #     raise BaseException("Dataset name not found!")
#     #
...
#         "parameters": {
#             "lxmert_lr": {"values": [1e-4, 3e-4, 5e-4]},
#             "dropout": {"values": [0.40, 0.45]},
#             "omega_v": {"values": [0.005, 0.01, 0.05]},
#             "omega_q": {"values": [0.05, 0.1, 0.3]}
#         },
#         # "early_terminate": {"type": "hyperband", "min_iter": 3}
#     }
#     sweep_id = wandb.sweep(sweep_config, project="CFR_VQA_PEmixer_2DRPE_alibi_localtest")
