...
# from lxrt.modeling_learnedPEmixer import BertFeatureExtraction
...
class WordEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        2D input: shape [batch_size, seq_len]
        3D input: shape [batch_size, num_objects, seq_len]
        """
        emb = self.emb(x)
        if 'c' in self.op:
            if len(x.size()) < 3:
...
class QuestionEmbedding(nn.Module):
    def __init__(self, in_dim, num_hid, nlayers, bidirect, dropout, rnn_type='GRU'):
        ...
        assert rnn_type == 'LSTM' or rnn_type == 'GRU'  # GRU
        ...
        self.in_dim = in_dim  # 600
        self.num_hid = num_hid  # 1024
